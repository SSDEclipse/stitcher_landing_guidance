...
def NormalizeQuaternion(Q0):
    dot_prod = np.dot(Q0, Q0)
    Q0Mag = dot_prod**(1/2)
    NormalizedQuaternion = Q0/Q0Mag
    return NormalizedQuaternion

def quaternionConjugate(Q0):
    w0 = Q0[0]
    x0 = Q0[1]
    y0 = Q0[2]
    z0 = Q0[3]
...
def convertVectorReferenceFrame(quat, inputVector):
    quatVector = np.array([0, inputVector[0], inputVector[1], inputVector[2]])
    temp = quaternion_multiply(quaternion_multiply(quat, quatVector), quaternionConjugate(quat))
    return np.array([temp[1], temp[2], temp[3]])
...
# Case 1 from Lossless Convexification of Nonconvex Control
...
# Case 2 from Lossless Convexification of Nonconvex Control
# Bound and Pointing Constraints of the Soft
# Landing Optimal Control Problem
...
# Case 1 from Convex Programming Approach to Powered Descent Guidance for Mars Landing
